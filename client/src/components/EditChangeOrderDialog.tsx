...
import type { Document, ProductionRateBlock } from "@shared/schema";
...
interface EditChangeOrderDialogProps {
  changeOrder: Document;
  parentDocumentId: number;
  originalTotalAmount: number;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSaved?: (updatedDoc: Document) => void;
...
export function EditChangeOrderDialog({
  ...
  const coSellRateSnapshot = useMemo<number | null>(() => {
    const content: any = (changeOrder as any)?.content || {};
    const snap = content.sellRateSnapshot;
  ...
  useEffect(() => {
    if (open && changeOrder) {
      ...
      const content = (changeOrder.content as any) || {};
      const items = content.items || [];
      const loadedItems: LineItem[] = items.map((item: any) => ({
        name: item.name || "",
        description: item.description || "",
        quantity: item.quantity || 1,
        unitPrice: typeof item.unitPrice === 'number' ? item.unitPrice / 100 : 0,
        total: typeof item.total === 'number' ? item.total / 100 : 0,
        taxable: item.taxable ?? false,
        isOptional: item.isOptional,
      }));
      const loadedBlocks: ProductionRateBlock[] = content.productionRateBlocks || [];
      const savedOrder: string[] | undefined = content.itemOrder;
      let rebuilt: COEntry[];
      if (savedOrder && Array.isArray(savedOrder)) {
        let bIdx = 0, iIdx = 0;
        rebuilt = [];
        for (const t of savedOrder) {
          if (t === 'block' && bIdx < loadedBlocks.length) rebuilt.push({ type: 'block', block: loadedBlocks[bIdx++] });
          else if (t === 'item' && iIdx < loadedItems.length) rebuilt.push({ type: 'item', item: loadedItems[iIdx++] });
        }
  ...
  }, [open, changeOrder?.id, changeOrder?.updatedAt, changeOrder?.title, JSON.stringify((changeOrder as any)?.content)]);
  ...
  const handleSave = async () => {
    if (!title.trim()) {
      toast({ title: "Please enter a title for the change order", variant: "destructive" });
      return;
    }
    if (entries.length === 0) {
    ...
    const preservedCoSnap = (changeOrder as any)?.content?.sellRateSnapshot;
    if (typeof preservedCoSnap === 'number' && preservedCoSnap > 0) {
      contentData.sellRateSnapshot = preservedCoSnap;
    } else if (coSellRateSnapshot != null && coSellRateSnapshot > 0) {
      contentData.sellRateSnapshot = coSellRateSnapshot;
    }

    setIsSaving(true);
    try {
      const res = await fetch(`/api/documents/${changeOrder.id}`, {
        method: 'PUT',
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/documents', parentDocumentId, 'change-orders'] });
      queryClient.invalidateQueries({ queryKey: ['/api/documents', changeOrder.id] });
      toast({ title: "Change Order Updated", description: "Your changes have been saved." });
      onOpenChange(false);
      onSaved?.(updatedDoc);
    } catch (err) {
  ...
  const handleBack = () => {
    const originalContent = (changeOrder.content as any) || {};
    const originalItemsCount = (originalContent.items || []).length;
    const originalBlocksCount = (originalContent.productionRateBlocks || []).length;
    const originalTitle = changeOrder.title || '';
    const hasChanges = title.trim() !== originalTitle || lineItems.length !== originalItemsCount || productionRateBlocks.length !== originalBlocksCount;
    ...
    <>
      {createPortal(
        ...
        <div
          ...
          data-testid="edit-change-order-page"
        ...
        >
            ...
            <div className="flex items-center justify-between gap-4 px-4 py-3 lg:px-8">
              <div className="flex items-center gap-3 min-w-0">
                <button
                  ...
                  onClick={handleBack}
                  data-testid="button-back-edit-change-order"
                >
                  Done
                </button>
                <h1 className="text-lg lg:text-2xl font-bold font-display truncate text-white">Edit Change Order</h1>
              </div>
              <Button onClick={handleSave} disabled={isSaving} className="bg-primary text-primary-foreground" data-testid="button-save-edit-change-order">
            ...
            <div className={`grid ${userTier === 'elite' ? 'grid-cols-3' : 'grid-cols-2'} gap-2 px-4 pb-3 lg:px-8`}>
              {userTier === 'elite' && (
                <button
                  ...
                  data-testid="button-edit-co-add-estimate"
                >
              ...
              <button
                ...
                data-testid="button-add-edit-co-item"
              >
              ...
              <button
                ...
                data-testid="button-edit-co-from-library"
              >
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    data-testid="input-edit-change-order-title"
                  />
                </div>
                ...
                <div className="pt-2 border-t space-y-3">
                  {entries.length === 0 ? (
                    <div className="text-center py-6 text-muted-foreground">
                      ...
                      <p className="text-sm mt-1">
                        {userTier === 'elite' ? 'Use "Add Estimate" or "Add Item"' : 'Tap "Add Item" to get started'}
                      </p>
                    </div>
                  ) : (
                    <>
                  ...
                  <div className="pt-3 border-t space-y-1">
                    {calculateTaxAmount() > 0 && (
                      <>
                        <div className="flex items-center justify-between text-sm text-muted-foreground" data-testid="text-edit-co-subtotal">
                          <span>Subtotal</span>
                          <span className="tabular-nums">{formatCurrency(calculateSubtotal())}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm text-muted-foreground" data-testid="text-edit-co-tax">
                    ...
                    <div className="flex items-center justify-between text-lg font-semibold">
                      ...
                      <span data-testid="text-edit-co-total">{formatCurrency(calculateTotal())}</span>
                    </div>
          ...
          {showRoomBuilder && createPortal(
            <div className="fixed inset-0 z-[10000] bg-background flex flex-col" data-testid="edit-co-fullscreen-room-builder">
                ...
                <div className="flex items-center justify-between px-3 py-2.5 lg:px-8">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      data-testid="button-edit-co-close-builder"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      data-testid="button-edit-co-save-estimate-header"
                    >
              ...
              <div className="flex-1 overflow-y-auto px-1.5 py-3 lg:px-8 lg:py-6">
                <div className="max-w-3xl mx-auto">
                  <RoomBuilder
                    sellRateSnapshot={coSellRateSnapshot}
                    builderRef={roomBuilderRef}
                    projectId={changeOrder.projectId || undefined}
                    taxRate={taxRate}
      ...
      {showServicePicker && createPortal(
        <div className="fixed inset-0 z-[10001] bg-black/60 flex items-center justify-center p-4 touch-none overscroll-none" onClick={() => setShowServicePicker(false)} onTouchMove={e => e.preventDefault()} data-testid="edit-co-service-picker-overlay">
          <div className="bg-background rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()} onTouchMove={e => e.stopPropagation()} data-testid="edit-co-service-picker-modal">
                ...
                return fullTypes.map(t => {
                  ...
                  return (
                    <button
                      ...
                      data-testid={`edit-co-service-option-${t}`}
                    >
            ...
            <div className="px-6 pb-5 pt-2">
              <Button variant="ghost" size="sm" className="w-full text-muted-foreground" onClick={() => setShowServicePicker(false)} data-testid="button-edit-co-cancel-service-picker">
                Cancel
              </Button>
            </div>
          </div>
      ...
      <AlertDialog open={showUnsavedWarning} onOpenChange={setShowUnsavedWarning}>
        <AlertDialogContent className="z-[10001]" overlayClassName="z-[10001]">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => { setShowUnsavedWarning(false); onOpenChange(false); }}
