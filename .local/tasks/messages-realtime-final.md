...
## Update — May 2026 (with explicit user approval)

User reported two regressions on top of the locked-down behavior and explicitly approved touching the LOCKED sections to fix them:

1. **Outbound bubble flicker** — typed a text, hit Send, bubble appeared, vanished for 1–2s, then reappeared. Root cause: iOS keyboard close on Send tap fires window focus, which triggered `refetchOnWindowFocus` on the active thread query. The in-flight stale GET clobbered the optimistic write before WS `sms.sent` reconciled.
   - Fix: `refetchOnWindowFocus: false` on the three thread queries (`useCommunications`, `by-phone`, `project-thread`). WS keeps the cache authoritative.
   - Belt-and-suspenders: `await queryClient.cancelQueries({ queryKey: cacheKey })` immediately before the optimistic `setQueryData` in `handleSend`, so any other in-flight fetch is killed and can't race the optimistic write.

2. **Inbound push delay** — push notification arrived, user tapped, /messages opened with the right thread selected, but the message bubble took ~5s to appear. Root cause: push payload only carried `{url, tag}`; when the app was cold-launched or resumed on cellular, the bubble had to wait for both WS reconnect AND the per-thread GET to complete.
   - Fix (server): extended `PushPayload` with `messageId`, `contactId`, `phoneNumber`, `projectId`, `timestamp`, `body` and stamped them into the FCM `data` payload AND the APNs custom userInfo at all 4 SMS push call sites (Twilio + OpenPhone, contact-known + phone-only branches).
   - Fix (client/main.tsx): `pushNotificationActionPerformed` now extracts the seed fields (plus `notification.body` as a fallback for the body) into the `__pushDeepLink` sessionStorage entry and includes them in the `native-push-tapped` event detail.
   - Fix (client/Messages.tsx): the LOCKED `handlePushTap` now seeds the per-thread cache with a synthetic inbound message **using the real DB messageId** before/alongside `setSelected`. The WS `message.created` handler dedupes against this id when WS finally reconnects, so there is no double-bubble. Same seed runs in the `replayFromBackup` cold-launch path.

Net effect: outbound bubble is sticky (no flicker); inbound bubble appears the instant the right pane mounts, with WS arriving later as silent reconciliation.
